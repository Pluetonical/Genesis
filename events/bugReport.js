...
module.exports = {
  ...
  async execute(message, client) {
    ...
    client.channels.cache.get('997002422041980938').send({ embeds: [new Discord.MessageEmbed().setTitle("<:atlanta_folder:601019084468912129> New Bug Report").setColor(0xffcd00).setDescription(`**Submitted by:** <@${message.author.id}> | ${message.author.tag}\n\n**Message:**\n\`\`\`${message.content}\`\`\`\n\n**Status:** -`).setFooter(`${message.author.id} | ${message.attachments.map(attachment => attachment.url)}`)], components: [button] }).then(msg => {
