...
module.exports = {
  ...
  async execute(message, client) {
    ...
    if (message.channel.id == '997002333886091314' || message.channel.type == "DM") {
      ...
      discordAccount?.send(`Hello, ${discordAccount}!\n**${args[0].toString()}** has requested to associate your Discord account with their Genesis account.\n\nPlease react with ✅ if you would like to continue and ❎ if you wouldn't like to do so.\n\n**Please respond within 5 minutes else the verification process will be automatically aborted.**`).then(botMessage => {
        ...
        botMessage.awaitReactions({ filter, max: 1, time: 300000, errors: ["time"] }).then(async collected => {
          if (collected.first().emoji.name === "✅") {
            ...
            discordAccount.roles.add(client.cache.get('997001983875616788').roles.cache.find(role => role.name === "Player")) 
              ...
              if (args[2].toString() == "Mvp" || args[2].toString() == "MvpPlus" || args[2].toString() == "Famous" || args[2].toString() == "Media") {
                if (args[2].toString() == "MvpPlus" || args[2].toString() == "Mvp") {
                  if (args[2].toString() == "Mvp") {
                    discordAccount.roles.add(client.guilds.cache.get('997001983875616788').roles.cache.find(role => role.name == "MVP"));
                  } else {
                    discordAccount.roles.add(client.guilds.cache.get('997001983875616788').roles.cache.find(role => role.name == "MVP+"));
                  }
                } else {
                  discordAccount.roles.add(client.guilds.cache.get('997001983875616788').roles.cache.find(role => role.name == roleName));
                }
              }
            ...
            **/
            ...
            discordAccount.roles.add(client.guilds.cache.get('997001983875616788').roles.cache.find(role => role.name == "Player"));
            message.guild.channels.cache.get('996156218282356777').send(`${discordAccount} has verified their account. Their username is ${args[0].toString()}.`);
