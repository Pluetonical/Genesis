...
module.exports = {
  ...
  async execute(message, client) {
    ...
    if (message.content == "") return message.channel.send("<:cross:883326239341965332> | You must include the offender's IGN and rule broken.").then(msg => { setTimeout(() => { msg.delete(); }, 10000); });
      ...
      client.channels.cache.get('997002389804548186').send({ embeds: [new Discord.MessageEmbed().setTitle("<:atlanta_folder:601019084468912129> New Player Report").setColor(0xffcd00).setDescription(`Submitted by: ${message.author.tag}\n\nDetails:\n${message.content}\n\nStatus: -`).setFooter(message.author.id)], components: [button] }).then(msg => {
        if (message.attachments.size !== 0) {
